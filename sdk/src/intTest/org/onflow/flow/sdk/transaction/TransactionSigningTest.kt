...
class TransactionSigningTest {
    ...
    fun `Can sign transactions`() {
        for (curve in arrayOf(
            SignatureAlgorithm.ECDSA_P256,
            SignatureAlgorithm.ECDSA_SECP256k1
        )) {
            val pk1 = Crypto.getSigner(Crypto.generateKeyPair(curve).private)
            val pk2 = Crypto.getSigner(Crypto.generateKeyPair(curve).private)
            val pk3 = Crypto.getSigner(Crypto.generateKeyPair(curve).private)
